...
def show_report():
    ...
    def generate_pdf():
        ...
        for qid in sorted(progress.get('answers', {}).keys(), key=int):
            ...
            pdf.multi_cell(0, 8, f"Question {qid}: {question_text}")
            ...
            pdf.set_font("Arial", '', 10)
            truncated_answer = answer_text[:400] + "..." if len(answer_text) > 400 else answer_text
        ...
        # FIX: Return the PDF bytes directly - removed .encode('latin-1')
    ...
    if st.button("📥 Generate & Download PDF Report", type="primary", use_container_width=True):
        with st.spinner("Generating PDF report..."):
            try:
                ...
                st.info("Try using simpler text or check for special characters in your answers.")
